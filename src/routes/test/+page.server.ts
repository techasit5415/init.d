...
export const actions: Actions = {
    ...
    create: async ({ request, locals }) => {
        ...
        const node = String(fd.get('node') ?? '').trim();
        const network = String(fd.get('network') ?? '').trim();
        const id = Number(fd.get('id'));

        if (!recordId) return fail(400, { error: 'Missing recordId.' });
        if (!node) return fail(400, { error: 'Missing node.' });
        if (!network) return fail(400, { error: 'Missing network.' });
        if (!Number.isInteger(id) || id < 1) return fail(400, { error: 'Missing id.' });

        try {
            const record = await locals.pb.collection('instances').getOne<LeaseInstance>(recordId);
            const detail = {
                ...record,
                rootfs: `local-lvm:${record.specs.disk}`
            };

            if (record.type === 'container') {
                if (detail.os_template === undefined) {
                    return fail(400, { error: 'Missing ostemplate for container.' });
                }
                console.log('Creating container with details:', detail, 'on node:', node, 'with network:', network, 'and id:', id);
                await createCT(detail, network, node, id);
            } else {
                await createVM(detail, network, node, id);
            }

            // await locals.pb.collection('instances').update(recordId, { status: 'completed' });

            return { ok: true, recordId };
        } catch (e) {
            console.error('create failed', e);
            return fail(500, { error: 'Could not create instance.', recordId });
        }
    },
